...
import { X, CheckCircle2, Clock, AlertCircle, BarChart3, TrendingUp } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-150">
      ...
      >
        ...
        <div className="p-5 overflow-y-auto flex-1 space-y-4">
          ...
          <div className="space-y-2 pt-2">
            ...
            <div className="space-y-2">
              {categoryCounts.filter(c => c.count > 0).map(cat => (
                <div key={cat.id} className="p-2.5 rounded-xl bg-neutral-900/80 border border-neutral-800 flex items-center justify-between text-xs">
                  <div className="flex items-center space-x-2">
                    <IconRenderer name={cat.icon} size={14} color={cat.color} />
                    <span className="text-neutral-200 font-medium">{cat.name}</span>
                  </div>
                  <span className="text-neutral-400 font-mono text-[11px]">
                    <strong className="text-red-400">{cat.completed}</strong> hechas / {cat.count} total
                  </span>
                </div>
              ))}
            </div>
          ...
          {/* Philosophy Note */}
          <div className="p-3.5 rounded-2xl bg-neutral-900 border border-red-500/20 text-xs text-neutral-300">
            <span className="font-black text-red-500 block mb-0.5">Regla de oro de FKUS:</span>
